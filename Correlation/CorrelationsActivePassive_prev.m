...
test_active = CorrelationsByAnimalBehavior(TNBehavior(:,2),[70,60,50]);

[passive_NCorr,~] = MungeCorr2Mat(test_passive.NCorrAll);
[active_NCorr, ~] = MungeCorr2Mat(test_active.NCorrAll);
...
passive2 = CorrelationsByAnimalBehavior(TNBehavior(:,1));
active2 = CorrelationsByAnimalBehavior(TNBehavior(:,2),[70,60,50]);
[passive_all2,passive_all2_mu] = MungeCorr2Mat(passive2.NCorrAll);
[active_all2, active_all2_mu] = MungeCorr2Mat(active2.NCorrAll);
...
out.All = NikCompareCorrs(passive_all2_mu,active_all2_mu,close_idx,PlotColor);
% figure;
% hold on
...
for lvl =1:length(LevelNames)
 [active_lvl,active_lvl_mu] = MungeCorr2Mat(active2.NCorr(:,lvl));
 
 LevelName = LevelNames{lvl};  
 out.(LevelName) =  NikCompareCorrs(passive_all2_mu,active_lvl_mu,close_idx,PlotColor);
 if SaveName
...
function [m,m_mu] = MungeCorr2Mat(corrs)
 ...
 m_mu = cell2mat(cellfun(@(x) nanmean(x)', corrs,'UniformOutput',0));
    ...
    function  NoiseCorrResults = NikCompareCorrs(rPDFF, rHDFF,close_idx,PlotColor)
