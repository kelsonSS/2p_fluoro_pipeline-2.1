function [IGClow,IGChigh] = FDRcontrolBHv5(Devtotal,DkSmth,alpha,Md,wftotal,Whc,Mlow)
...
for t = 1:N
    ...
    for ee = 1:numel(siglind)
        ...
        if sign(sum(wslow)) ~=0
            ...
            IGClow(rows(ee),cols(ee),t) = sign(sum(wslow))*IGC(rows(ee),cols(ee),t);
        else
            IGClow(rows(ee),cols(ee),t) = IGC(rows(ee),cols(ee),t); % No sgn change if over regularized
        end
        if sign(sum(wshigh)) ~=0            
            IGChigh(rows(ee),cols(ee),t) = sign(sum(wshigh))*IGC(rows(ee),cols(ee),t);
        else
