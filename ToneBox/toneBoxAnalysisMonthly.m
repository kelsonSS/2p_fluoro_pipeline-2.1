function [bhvOutput] = toneBoxAnalysisMonthly(fileName, trials, Opt, cfact, useDayTrials)
...
if phaseChoice ~=4
    ...
    targetResp(:,1) = TrialTime;
    targetResp(:,2) = LL;
    plot(targetResp(:,1),targetResp(:,2),'b','linewidth',2);
else
    ...
    nontargetResp(:,1) = TrialTime;
    nontargetResp(:,2) = LL;
    plot(nontargetResp(:,1),nontargetResp(:,2),'r','linewidth',2);
end
xlim(Tidx);
hold on
title('Lick-o-gram')
...
cla
earlyRate = 100*earlyCount/totalTrials;
hitRate = 100*hitCount/totalTrials;
falseAlarmRate = 100*falseAlarmCount/totalTrials;
bar(1,earlyRate,'facecolor','g','edgecolor','g');
hold on
bar(2,hitRate,'facecolor','b','edgecolor','none');
bar(3,falseAlarmRate,'facecolor','r','edgecolor','none');
...
optTimesTemp = timeStamp(Rtrials,1);
formatIn = 'MM/dd/yyyy HH:mm:ss';
optTimes=datetime;
...
if ~exist('earlyRate', 'var')
    bhvOutput.earlyRate = NaN;
else
    bhvOutput.earlyRate = earlyRate;
end
if ~exist('hitRate', 'var')
    bhvOutput.hitRate = NaN;
else
    bhvOutput.hitRate = hitRate;
end
if ~exist('falseAlarmRate', 'var')
    bhvOutput.falseAlarmRate = NaN;
else
    bhvOutput.falseAlarmRate = falseAlarmRate;
end
if ~exist('dprime', 'var') || isinf(dprime)
    bhvOutput.dprime = NaN;
else
    bhvOutput.dprime = dprime;
end
if ~exist('targetResp', 'var')
    bhvOutput.targetResp = NaN(length(TrialTime),2);
else
    bhvOutput.targetResp = targetResp;
end
if ~exist('nontargetResp', 'var')
    bhvOutput.nontargetResp = NaN(length(TrialTime),2);
else
    bhvOutput.nontargetResp = nontargetResp;
end
bhvOutput.t = t;
bhvOutput.lickResponseE = lickResponseE;
bhvOutput.lickResponseH = lickResponseH;
bhvOutput.lickResponseF = lickResponseF;
bhvOutput.out = out;