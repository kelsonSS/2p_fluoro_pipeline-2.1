function BayesModels = BayesClassiferActive_Degraded(Active_All,reduced_n,absolute_value_flag)
...
%end 
if ~ exist('reduced_n','var')  
    reduced_n = 20  % number of neurons to pick from 
end

if ~exist('absolute_value_flag','var')
    absolute_value_flag = false;
end 
...
AllBehavior = AllBehavior(1:trials)';
 ...
 for run = 1: ceil(num_reps/3)  
         ...
         parfor neurons = 2:num_neurons
             ...
             rand_idx = randi(reduced_n,neurons,1);
             mdlData = squeeze(nanmean(AllData(tone_on:tone_off,1:trials,rand_idx)));
             mdlData = degradeData(mdlData,reduced_n);
             %% Modeling Hits
...
function data = degradeData(data,reduced_n)

if size(data,2) <= reduced_n
    return
end 

degraded_N = size(data,2) - reduced_n;
trials  = size(data,1);

% random value between 50-150% for each neuron being degraded%

rand_vals = (rand(trials,degraded_N)) + .5 ;

 
 data(:,reduced_n+1:end) =  data(:,reduced_n+1:end).* rand_vals;
