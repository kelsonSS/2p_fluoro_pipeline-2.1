function Out = CompareBayes(g1,g2,behaveField,GroupNames,Levels,SaveName)

% unpack
time_fld = ['TimeLossTotal_' behaveField];
g1_time = squeeze(g1.stats.(time_fld)(:,end,:));
g2_time = squeeze(g2..(time_fld)(:,end,:));

neuron_fld = ['NumbersLossTotal_' behaveField];
g1_neurons = squeeze(g1.(neuron_fld)(:,end,:));
g2_neurons = squeeze(g2.ClassesTonesLossTotal(:,end,:));

neuron_fld = ['TimeLosslLvL_' behaveField];
g1_lvls = squeeze(max(g1.(neuron_fld)(:,:,end,:)));
