...
out = struct();
    if exist('manual_colors','var')
        ...
        % colors = {'w','r','o','y','c','b'};for reference 
...
for expt_idx = 1:length(expts)
 ...
 try
        load(fullfile(expt_directory,'AvgGreenImage.mat'));
           ...
           img =fread(fopen(f,'r+'), 512*512*100,'uint16=>uint16');fclose(f);
          
           img = reshape(img,512,512,100);
           avgGreenFilteredImage = squeeze(nanmean(img,3));
           clear img
        end 
 ...
 for current_cluster = 1:max(classes)+1 % classes are 0-indexed 
     ...
     cells_to_plot = find(classes == current_cluster - 1);
...
%     
 ...
 print(gcf,'-dpsc','-append','\\vault3\data\kelson\CellClusters.ps')
% 
%  figure
